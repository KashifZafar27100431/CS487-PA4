...
def report_activity(order: dict) -> str:
    ...
    safe_order_id = re.sub(r"[^a-z0-9-]+", "-", order_id.lower()).strip("-") or "order"
    name     = f"ci-report-{safe_order_id[:45]}"
    ...
    if state != "Succeeded":
        raise RuntimeError(f"Report container {name} ended with state {state}")
